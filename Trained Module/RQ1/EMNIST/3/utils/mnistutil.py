...
#            y_test[i]=y_test[i]-1
    ...
    # the data, split between train and test sets
        x_train = self.load_data('emnist-train-imgs.npz')
        x_test = self.load_data('emnist-test-imgs.npz')
        y_train = self.load_data('emnist-train-labels.npz')
        for i in range(0,len(y_train)):
            y_train[i]=y_train[i]-1
        y_test = self.load_data('emnist-test-labels.npz')
        for i in range(0,len(y_test)):
            y_test[i]=y_test[i]-1
        x_zo = []
        y_zo = []
